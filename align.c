 ...
 */
	...
	// Pat_number contine il numero di pattern totali tra generati e campionati
	// Per ognuno 
	/*
	pat_found è un array utilizzato per memorizzare 
	le posizioni di inizio dei pattern trovati nella sequenza principale. 
	Ogni elemento rappresenta la posizione di inizio del pattern, e l'indice rappresenta il numero del pattern 
	contiene  NOT_FOUND se il pattern non è stato trovato
	
	*/
	//Scorre tutta la sequenza e per ogni posizione la inizializza a NOT_FOUND
	for( ind=0; ind<pat_number; ind++) {
		pat_found[ind] = (unsigned long)NOT_FOUND;
	}

	// sew_length è la lunghezza della sequenza DNA
	// seq_matches è un array di interi lungo quanto il DNA e in ogni posizione contiene il numero di pattern trovati
	// ad esempio se la sequenza è CCACCTA e il pattern è CC allora seq_matches[0] = 1, seq_matches[1] = 1, seq_matches[2] = 0, seq_matches[3] = 0, seq_matches[4] = 0, seq_matches[5] = 0, seq_matches[6] = 0 

	// Inizializza tutto a NOT_FOUND
	...
	for( pat=0; pat < pat_number; pat++ ) {
		...
		// Per ogni posizione della sequenza
		// path_length[pat] è la lunghezza del pattern pat
		for( start=0; start <= seq_length - pat_length[pat]; start++) {

			// Per ogni elemento del pattern
			for( lind=0; lind<pat_length[pat]; lind++) {
				
				// Sequence è la nostra sequenza DNA
				// Pattern è un array di pattern che dobbiamo cercare nel DNA
				// Sostanzialmente stiamo verificando se il pattern è presente nella sequenza DNA passando per ogni posizione del dna
				// Va avanti finche non trova due elementi diversi
				if ( sequence[start + lind] != pattern[pat][lind] ) break;
			}
			// Se siamo arrivati alla fine del pattern ha trovato un match
			// Immagazzina la posizione di inizio del pattern
		...
		// Se quindi è stato trovato un match incrementa il numero di match trovati
	...
	// Per ogni pattern trovato tira fuori checksum_found con calcoli strani
	for( ind=0; ind < pat_number; ind++) {
		if ( pat_found[ind] != (unsigned long)NOT_FOUND )
			checksum_found = ( checksum_found + pat_found[ind] ) % CHECKSUM_MAX;
	}
	// Per ogni posizione della sequenza tira fuori checksum_matches con calcoli strani
	for( lind=0; lind < seq_length; lind++) {
