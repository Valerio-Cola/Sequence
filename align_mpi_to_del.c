 ...
 */
	...
	// Ognuno si prende tot pattern
	/* Si tiene traccia del primo e dell'ultimo pattern
	int my_patterns = pat_number / size;
	int remainder = pat_number % size;
	
	int my_first_pattern;
	int my_last_pattern;
	if(rank < remainder){
		my_first_pattern = rank * my_patterns + rank;
		my_last_pattern = my_first_pattern + my_patterns;
	}else{
		my_first_pattern = rank * my_patterns + remainder;
		my_last_pattern = my_first_pattern + my_patterns -1;
	}

	int my_first_pattern = rank * my_patterns + (rank < remainder ? rank : remainder);
	int my_last_pattern = my_first_pattern + my_patterns - 1 + (rank < remainder ? 1 : 0);
*/
	...
	MPI_Barrier( MPI_COMM_WORLD );
	printf("Rank %d: First pattern: %d, Last pattern: %d\n", rank, my_first_pattern, my_last_pattern);
	/*

	printf("Rank %d: First pattern: %d, Last pattern: %d\n", rank, my_first_pattern, my_last_pattern);
	for (int i = my_first_pattern; i <= my_last_pattern; i++) {
		printf("nRank %d Pattern[%d]: ",rank, i);
		for (unsigned long j = 0; j < pat_length[i]; j++) {
	...
	*/
	...
	/*Print seq_matches and pat_found from each rank 
	MPI_Barrier( MPI_COMM_WORLD );
	
	printf("\nRank %d seq_matches: ", rank);
	for (lind = 0; lind < seq_length; lind++) {
		printf("%d ", seq_matches[lind]);
	}
	MPI_Barrier( MPI_COMM_WORLD );
	printf("\nRank %d pat_found: ", rank);
	for (ind = my_first_pattern; ind <= my_last_pattern; ind++) {
		printf("%lu ", pat_found[ind]);
	}
	printf("\n");
	MPI_Barrier( MPI_COMM_WORLD );
	*/
